...
const StatusBar: React.FC = () => {
  ...
  return (
    <header
      className={`slate-status print-hidden fixed inset-x-0 top-0 z-50 h-10 border-b border-zelda-light-blue/25 bg-zelda-dark/70 backdrop-blur-sm ${
        EFFECTS.deviceFrame ? "rounded-t-[0.85rem]" : ""
      }`}
    >
      <div
        className={`mx-auto flex h-full max-w-6xl items-center justify-between gap-3 ${
          EFFECTS.deviceFrame ? "px-8" : "px-4 sm:px-6"
        }`}
      >
        ...
        <div className="flex items-center gap-3 sm:gap-5">
          ...
          <a
            ...
            title="Send a signal"
            className="text-zelda-light-blue/80 transition-colors hover:text-zelda-gold"
          >
            <RuneIcon name="signal" className="h-4 w-4" />
          </a>
